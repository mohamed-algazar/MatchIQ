...
class CameraMovementEstimator():
    def __init__(self, frame):
        ...
        height, width = first_frame_grayscale.shape[:2]
        mask_features = np.zeros_like(first_frame_grayscale)

        edge_width = max(20, width // 20)
        mask_features[:, :edge_width] = 1
        mask_features[:, max(0, width - edge_width):] = 1
            ...
            if new_features is not None and status is not None:
                ...
                for new, old in zip(good_new, good_old):
                    ...
                    if distance > self.minimum_distance:
                        dx, dy = measure_xy_distance(old_point, new_point)
                        movement_vectors.append([dx, dy])

            if movement_vectors:
                movement_vectors = np.array(movement_vectors)
                camera_movement_x = float(np.median(movement_vectors[:, 0]))
                camera_movement_y = float(np.median(movement_vectors[:, 1]))
                camera_movement[frame_num] = [camera_movement_x, camera_movement_y]

            # Refresh good features for the next step
            old_gray = frame_gray.copy()
            old_features = cv2.goodFeaturesToTrack(old_gray, **self.features)
